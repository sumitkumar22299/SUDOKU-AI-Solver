...
try:
    ...
    model = load_model('ocr/model1.hdf5')
    img_dims = 32
...
def process(img):
    #   cv2.getStructuringElement () Returns the specified shape and size of the structural elements.
    #   The first parameter to this function shows the shape of the core, there are three kinds of shapes can be selected.
    #   Rectangular: MORPH_RECT;
# #     The second and third parameters are the size and location of the kernel anchor.
# kernel: The kernel to be scanned through the image
# anchor: The position of the anchor relative to its kernel. The location Point(-1, -1) indicates the center by default
# What is a kernel?
# A kernel is essentially a fixed size array of numerical coefficients 
# along with an anchor point in that array, which is typically located at the center.


    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
    # MORPH_ELLIPSE
    # MORPH_CROSS
    # basically You just pass the shape and size of the kernel, you get the desired kernel.
    # [1,1]
    # [1,1]
...
#    more than (sometimes less than) the threshold value. 
    ...
    #  But it may not be good in all the conditions where image has different lighting 
    # conditions in different areas. In that case, we go for adaptive thresholding. 
    ...
    # 11 ----------  Block Size - It decides the size of neighbourhood area.
    ...
    # Inversion of input array elements. 0 to 1 AND 1 to 0 .
...
#  followed by dilation. Because, erosion removes white noises, but it also shrinks our object. So we dilate it. 
